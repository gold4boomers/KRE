...
class KURecorder {
    ...
    rec_receiveMessage (event) {
        ...
        if (!childFrame && event.source) {
            try{
                ...
                //Failed to read a named property 'frameElement' from 'Window': Blocked a frame with origin
                //Skip this error
                return true;    
            }
        }
        ...
        try {
            ...
            action["actionData"] = "";
            var json = mapDOMForRecord(action, childFrame, this.window);
            if (json) 
    ...
    rec_createInfoDiv () {
        addCustomStyle();
        this.rec_infoDiv = this.window.document.createElement('katalon-div');
    ...
    rec_createXpathDiv () {
        this.rec_elementInfoDiv = this.window.document.createElement('katalon-div');
    ...
    processOnInputChangeTarget(selectedElement){
        ...
        let isRichTextEditor = selectedElement.attributes.contenteditable?.value === "true";
        var isRecorded = (elementTagName === 'input' && 
                (elementTypeName && KURecorder.INPUT_TYPE_INPUT_EVENT.indexOf(elementTypeName) !== -1)) 
                || (elementTagName === 'textarea' || isRichTextEditor);
        if (!isRecorded) {
            return;
        }
        ...
        if (isRichTextEditor) {
              // Remove xpath is generated in iframe by Katalon.
              if (document.getElementById('katalon')) {
                action["actionData"] = selectedElement.innerHTML.replace(document.getElementById('katalon').outerHTML, '');
            } else {
    ...
    processOnChangeTarget (selectedElement) {
        ...
        if (selectedElement.tagName.toLowerCase() == 'select') {
            // To improve performance and prevent duplicated data
            var oldValue = selectedElement.oldValue;
            var newValue = this.rec_getSelectValues(selectedElement)
            if (this.compareArrays(oldValue, newValue)) {
                return;
            }
            action["actionData"] = {};
    ...
    mouse_over(event) {
        ...
        if (selectedElement != undefined && selectedElement.tagName == "SLOT") {
            return
        }
        ...
        if (selectedElement.shadowRoot instanceof ShadowRoot) {
            ...
            selectedElement.shadowRoot.addEventListener('change', this.select_change, true);
            return;
        }
    }
    select_change(event) {
            ...
            if (selectedElement.tagName) {
                ...
                if ('select' == tagName) {
                    if (!selectedElement.multiple) {
                        ...
                        this.processOnChangeTarget(selectedElement);
                    } else {
