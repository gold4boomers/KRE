...
KURecorder.addEventHandler('type', 'keydown', function (event) {
    typeTarget = this.hoveringElement;
    // In the rich text editor, the "enter press" does not listen to the "input" event. Fix ticket STUDIO-4831
    let isRichText = typeTarget.getAttribute('contenteditable') === 'true'
    if (typeTarget && event.key === 'Enter' && isRichText) {
