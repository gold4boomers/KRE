...
browser.tabs.onCreated.addListener(function (tab) {
    ...
    // Only record the new tab that's triggered from new tab button or hotkey.
    var url = tab.pendingUrl != null ? tab.pendingUrl : tab.url
    if (!NEW_TAB_REGEX.test(url)) {
        return;
    }
    ...
    clientSocket.send("elementAction" + "=" + encodeURIComponent(JSON.stringify(object)));
});
    ...
    if (mode == 'INSPECT') {
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
            if(tabs && tabs[0] && tabs[0].id){
                // Check title != "" because when user open new tab this tab still loading and user can not do anything
                // In this case we just send action not including windowID
                // After page loaded successfully, user can interact this page and we will send windowId to notify switch new tab in this case
                if (tabs[0].title != "") {
